...
```text
โจทย์ให้คำนวนคะเเนนจากทั้งหมด 3 คนซึ่งเเต่ละคนใส่คะเเนนได้เเค่ 0-10 เท่านั้นเป็นจำนวนจริง เเล้วตัดสินว่ามีโอกาสที่คะเเนนสูงสุด (max) มากกว่าคะเเนนต่ำสุด (min) จะห่างกันเกิน 2 คะแนนหรือไม่ ถ้าใช่เเสดง Surprising ถ้าไม่ใช่ Not surprising

Input :
2 บรรทัด
- คะเเนนผลรวมทั้งสามคน
- คะเเนนสูงสุดของสามคนนี้

Output: 
Surprising หรือ Not surprising

Constraints :
คะแนนแต่ละคนอยู่ในช่วง 0-10 และเป็นจำนวนจริง (มีทศนิยมได้)
```
...
```text
1. รับค่าคะเเนนรวมเเล้วเก็บในตัวเเปร sum_input
2. รับค่าคะเเนนสูงสุดจากสามคนเก็บในตัวเเปร max_input

3. เขียนสมการเช็คว่า sum - max <= max หรือไม่หากน้อยกว่าเราสามารถกำหนดตัวเเปรใหม่ min_set = 0 ได้เลย
เพราะว่า sum - max คือ คะเเนนของคนเเรกกับคนที่สอง (ที่ไม่ใช่ของคนที่ได้ max) ถ้ามันน้อยกว่า 0 หรือเท่ากับ 0 เราสามารถโยนคะเเนนให้คนเเรก = max ได้เลยเเล้วคนที่สองเป็น 0 ได้เลย

4. เขียน else ในกรณีที่ sum - max เเล้วมันมากกว่า max หรือ a+b > max (a คือคนที่ 1, b คือคนที่ 2) ดังนั้นเราไม่สามารถโยนให้ใครคนนึงเป็น 0 ได้ เราจึงต้องกำหนด min_set = (a+b - max) - max เพราะว่าเราสามารถกำหนดให้คนใดคนนึงในคนที่ 1, 2 = max ได้เลย เพราะว่า a+b มันมากกว่า max ไปเเล้ว

5. เช็คว่า max_input - min_set มากกว่า 2 หรือไม่ (ใช้ > ไม่ใช่ >= เพราะโจทย์บอกว่าเกิน 2 คะแนน)

6. เเสดงผล Surprising ถ้าคะเเนนห่างกันมากกว่า 2  ถ้าไม่เเสดงผล Not surprising
```
...
```text
เหมือนกับเเผนเเรก เพราะ ลองคำนวนดูเเล้วได้ตรงตามกับสมการที่คิดไว้
```
...
```text
ตรวจสอบกรณีห่างกันเท่ากับ 2 พอดี
```
...
```text
ตรวจสอบกรณีห่างกันเกิน 2 แบบหลักทศนิยม
```
