...
```text
โจทย์ต้องการให้เขียนโปรเเกรมรับค่าจำนวนเต็มบวก จากนั้นหาว่าตัวนั้น ถึง 0 มีเลขใดบ้างที่หาร 10 ลงตัว

Input :
N เป็นจำนวนเต็มบวก

Output :
เเสดงจำนวนเต็มบวกตั้งเเต่ N ถึง 0 ที่หาร 10 ลงตัว โดยเเสดงในบรรทัดเดียว เเละคั่นข้อมูลด้วย " "

Constraints :
การเเสดง output ต้องเป็นบรรทัดเดียวเเละคั่นข้อมูลด้วย " "
```
...
```text
1. รับ input จำนวนเต็มบวกเก็บไว้ในตัวเเปร N

2. สร้างตัวเเปร count เก็บค่า string เปล่าๆ " "

3. จากนั้นใช้ for loop โดยเริ่มต้นที่ N จบที่ -1 เเละ step คือ -1 เพราะเราต้องการให้ลดลงเรื่อยๆ ทีละ 1

4. เขียน if เช็คใน loop ว่าถ้า i % 10 เเล้วหารลงตัวให้เก็บไว้ในตัวเเปร count
...
```text
ติดปัญหาตรงที่มีช่องคั่นข้อมูลตั้งเเต่ตัวเเรกที่หาร 10 ลงตัวทั้งที่ไม่ควรจะมี ผมเลยเขียนเงื่อนไขเพิ่มเข้าไป
...
5. เขียน if เช็คต่อว่า ถ้า count เป็น string เปล่าหรือเป็น false ให้ + str(i) เข้าไปใน count ได้เลย 
ถ้าไม่ใช่ให้บวก str(" " + str(i)) เป็นการคั่นด้วยช่องว่าง
...
```text
อยากตรวจสอบว่าตรงกับ testcase ใน i judge หรือไม่
```
...
```text
ตรวจสอบว่าถ้า input หารสิบไม่ลงตัว โปรเเกรมยังจะทำงานอยู่หรือไม่
```
...
```text
ตรวจสอบว่า input ติดลบเเล้ว output จะไม่สามารถคำนวนได้ใช่หรือไม่
```
